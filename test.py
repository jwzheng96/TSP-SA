...
# 种群初始化编码(生成基因的随机排列)
def init_chromos(start=-1, gene_len=-1, population_num=-1):
    chroms = []  # 所有种群
...
def cross(chroms, cross_pro):
    ...
    return chroms
# 变异操作(随机调换单个染色体的基因位置)
def mutation(chroms, mutation_pro):
    n = len(chroms)
    gens_len = len(chroms[0])
    for i in range(n):
...
def test(data):
    sys.stdout = open("test_result.txt", "w")
    max_evolution_num = 500  # 最大进化代数
    population_num = 1000  # 种群数目
    cross_pro = 0.6  # 交叉概率
    mutation_pro = 0.2  # 变异概率
    citys = data # [(2, 6), (2, 4), (1, 3), (4, 6), (5, 5), (4, 4), (6, 4), (3, 2)]  # 城市坐标
    print('type of cities: ', type(citys))
    print('type of cities[0]: ', type(citys[0]))
    ...
    for step in range(max_evolution_num):
        ...
        chroms = init_chromos(start=start, gene_len=len(citys), population_num=population_num)  # 种群初始化，得到所有种群
        chroms = choice(citys, chroms)  # 选择操作，选择出每个种群的精英染色体
        chroms = cross(chroms, cross_pro)  # 交叉操作，两个染色体互相杂交产生新的染色体
        chroms = mutation(chroms, mutation_pro)  # 变异操作，单个染色体变异
